 ...
 * except for SimpleDialogs.prompt(), which uses the JavaFX
 * TextInputDialog.
...
public class SimpleDialogs {
    ...
    /**
     * Shows a dialog containing an arbitrary node as content. The node
     * is
     * assumed to do some sort of input. The dialog has "Cancel" and "OK
     * buttons.
     * The title of the dialog is "Input is Requested".
     * 
     * @param inputNode          the content node for the dialog.
     * 
     * @param headerText         text that appears above the content node in the
     *                           dialog;
     *                           can be null.
     * @param testForErrorString a Supplier<String> that performs an error
     *                           check on the input in the content node. This is
     *                           called when
     *                           the
     *                           user clicks "OK", before closing the dialog box. It
     *                           must return
     *                           a null
     *                           value if the input is good; otherwise, it must
     *                           return an error
     *                           string. When the value is non-null, the error
     *                           string is shown
     *                           to the user, and the dialog does not close. Cannot
     *                           be null!
     * @return true if the user dismisses the dialog with OK, and false
     *         otherwise.
     *         If the value is true, then the input in the content node has
     *         passed
     *         the error check.
     */
     ...
     *              automatically wrapped to a maximum width of 450 pixels. There
     *              is
    ...
    /**
     * Shows an input dialog box where the user can enter one line of
     * text.
     ...
     *                     headerText for the dialog. If it is to be displayed as
     *                     more
     *                     than
     * 
     *                     one line, it should contain \n characters to separate the
     *                     lines.
     *                     Should not be null.
     * @param title        text to appear in the title of the dialog box; can be
     *                     null
     * @param defaultValue if non-null, this is the initial text in the
     *                     input box
     * @return null if the user cancels the dialog, or the contents of the
     *         input
     *         box if the user dismisses the dialog with the OK button. Note
     *         that
     *         the return value can be an empty string, if the user hits "OK"
     *         while
    ...
    /**
     * Shows a dialog box with a message and three buttons: "yes", "no",
     * and
     * "cancel". The only thing the user can do is click one of the
     * buttons.
     * 
     ...
     *                contain \n characters to separate the lines. Should not be
     *                null.
     * @param title   text to appear in the title of the dialog box; can be
     *                null.
     * 
     * @return "yes", "no", or "cancel". The return value is always one
     *         of these
     *         three strings, depending on which button the user clicks to
     *         dismiss
     *         the dialog box. If the user closes the dialog box in some
     *         other
    ...
    /**
     * Shows a dialog box containing a simple color chooser pane that the
     * user
     * can manipulate to select a color. The dialog box has an OK button
     * and
     * a "Cancel" button.
     * 
     * @param initialColor the color that is initially selected in the
     *                     dialog.
     *                     If the value is null, the initial color is black.
     * @param headerText   text to be shown in the dialog above the color
     *                     chooser
     *                     pane. Can be null. For multi-line text, the \n character
     *                     should
     *                     be included in the string to separate the lines.
     * @return null if the user cancels the dialog, or the color that is
     *         selected
     * 
     *         in the color chooser pane if the user dismisses the dialog box
     *         by
     ...
     * to set the red, green, blue, hue, brightness, and saturation
     * components
    ...
    private static class ColorChooserPane extends GridPane {
        private Slider hueSlider, brightnessSlider, saturationSlider; // Sliders to control color components. redSlider,
                                                                      // greenSlider, blueSlider;

        private Label hueLabel, brightnessLabel, saturationLabel;
        // For displaying color component values. redLabel, greenLabel, blueLabel;
        ...
        public ColorChooserPane(Color initialColor) {
            /*
             * Create Sliders with possible values from 0 to 1, or 0 to
             * 
             * 360 for hue.
             */
            ...
            root.setStyle(
                    "-fx-padding:5px; -fx-border-color:darkblue; -fx-border-width:2px; -fx-background-color:#DDF");
